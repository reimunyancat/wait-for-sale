...
def build_realtime_features_multi(appid: str, days_list: list) -> Optional[list]:
    """
    predict_range용: ITAD 데이터를 1번만 받고, days_list의 각 days_ahead에 대해
    피처 벡터를 생성해서 리스트로 반환합니다.
    """
    client = ItadApiClient()
    model_dir = Path(__file__).parent
    ...
    latest_base = df.sort_values('date').iloc[-1].to_dict()
    ...
    for days_ahead in days_list:
        latest = dict(latest_base)
        if days_ahead > 0:
            target_date = date.today() + timedelta(days=days_ahead)
            latest['days_to_next_major_sale'] = days_to_next_major_sale(target_date)
            latest['is_major_sale_period'] = is_major_sale_period(target_date)
            latest['month'] = target_date.month
            latest['dayofweek'] = target_date.weekday()
            latest['dayofyear'] = target_date.timetuple().tm_yday
            if latest.get('days_since_last_sale', -1) >= 0:
                latest['days_since_last_sale'] = latest['days_since_last_sale'] + days_ahead
            if latest.get('days_since_release', -1) >= 0:
                latest['days_since_release'] = latest['days_since_release'] + days_ahead
...
def build_realtime_features(appid: str, days_ahead: int = 0) -> Optional[np.ndarray]:
    """
    실시간 예측용: 특정 appid에 대해 현재 날짜 기준 피처 벡터를 생성합니다.
    days_ahead > 0 이면 오늘 날짜 + days_ahead 기준으로 날짜 피처를 재계산합니다.
    학습된 스케일러 경로에서 로드합니다.
    """
    client = ItadApiClient()
    model_dir = Path(__file__).parent
    ...
    if days_ahead > 0:
        ...
        latest['dayofweek'] = target_date.weekday()
        latest['dayofyear'] = target_date.timetuple().tm_yday
        # days_since_last_sale, days_since_release도 조정
        if latest.get('days_since_last_sale', -1) >= 0:
            latest['days_since_last_sale'] = latest['days_since_last_sale'] + days_ahead
        if latest.get('days_since_release', -1) >= 0:
            latest['days_since_release'] = latest['days_since_release'] + days_ahead
